...
Suppose our current state is in $$q_0$$, target state is $$q_1$$, eqn. \eqref{detbal} is
$$
\begin{equation}
  p_t(q_0 \rightarrow q_1) p(q_0) = p_t(q_1 \rightarrow q_0) p(q_1)
\end{equation}
...
Suppose there is one single $$p_0$$ to satisfy that $$p_0,q_0$$ goes to $$p_1,q_1$$ after Hamitonian process. Then the LHS of eqn. \eqref{detbal} becomes:
$$
\begin{equation}
